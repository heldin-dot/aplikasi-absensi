...
function qzDonePrinting() {
    // Alert error, if any
    if (qz.getException()) {
        msgbox_warning('Error printing:\n\n\t' + qz.getException().getLocalizedMessage());
        qz.clearException();
        return;
    }

    // Alert success message
    msgbox_success('Successfully sent print data to "' + qz.getPrinter() + '" queue.');
}
...
function findPrinter(name) {
    ...
    if (isLoaded()) {
        ...
        window['qzDoneFinding'] = function () {
            ...
            // Alert the printer name to user
            if(printer !== null) {
                msgbox_success('Printer found: "' + printer + '" after searching for "' + p.value + '"');
            } else {
                msgbox_warning('Printer "' + p.value + '" not found.');
            }
